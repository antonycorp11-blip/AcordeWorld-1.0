...
x = 40
while x < 1500:
    add('casa2_03', x, ESTRADA_Y, 1.0, False); x += 225
while x < W + 200:
    add('f3_01', x, ESTRADA_Y, 1.0, False); x += 479
...
add('sagrado1_04', 2450, 1620, 1.15)
add('sagrado1_05', 2650, 1700, .9)
aglomerado(MATO + ['f2_09'], 2450, 1620, 8, 230, (.7, 1.05), 34)
add('mata2_01', 2260, 1560, 1.0, espaco=40)
add('mata2_03', 2690, 1560, .9, espaco=40)

# afloramento de rocha
for cx, cy in [(3300, 700), (3420, 760), (3220, 790)]:
...
for _ in range(9):
    cx, cy = random.randint(1800, 4000), random.randint(400, H - 400)
...
for bx in range(4500, 5900, 190):
    add('sagrado2_02', bx, CY - 280, 1.0, False, espaco=40)
add('sagrado2_03', 5980, CY - 270, 1.0, False)

# pedras da partitura, o motivo do lugar existir
add('lapide2_01', 4600, CY + 330, 1.05, espaco=60)
add('lapide2_03', 5700, CY + 300, 1.0, espaco=60)
add('lapide2_02', 5150, CY + 430, 1.0, espaco=55)

# cachoeiras na quina do mundo
add('magico3_02', 5950, 620, 1.1, False, espaco=70)
add('magico3_01', 5700, 520, .95, False, espaco=60)
add('magico2_01', 5850, 800, 1.0)

# a ponte, onde a estrada encontra o bosque sagrado
add('vila2_04', 4250, ESTRADA_Y, 1.0, False)

# ════════════════════════════════════════════════════════════════════════════════
cfg = json.load(open('assets/mundo/mundo.json'))
cfg['nome'] = 'O Vale das Claves'
cfg['cols'], cfg['rows'] = COLS, ROWS
cfg['spawn'] = {'x': 300, 'y': ESTRADA_Y + 70}
cfg['props'] = props
